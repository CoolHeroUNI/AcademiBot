...
const { FB, Dialogflow } = require("../src/Instances");
const { Parametros, Facultad, Ciclo, Especialidad, Recurso } = require("../schema");
...
async function executePetition(user, petition, text) {
  const userId = user.get('canal').get('codigo');
  const canalId = user.get('canal').get('id');
  switch (petition) {
    case 'Meme':
      let memes = (await findRecursos(canalId, media_folder + '/memes')).slice(0, 1 + Math.floor(Math.random() * 2));
