...
async function findUsuario(channelName, idCode, t, logging = console.log) {
  ...
  const usuario = await Usuario.findByPk(cuenta.get('usuario_id'), {
    include: [
      { model: Usuario_info, as: 'info' },
      { model: Usuario_solicitud, as: 'solicitud'},
      { model: Usuario_donacion, as: 'donacion' },
      { model: UsuarioCanal_mensajeria, as: 'canal', where: { canal_mensajeria_id: channelName } }
    ],
...
async function findRecurso(channelName, ruta, es_visible = true) {
  ...
  recurso.setDataValue('canal_mensajeria', recurso.get('canal_mensajeria')[0]);
  return recurso;
}
